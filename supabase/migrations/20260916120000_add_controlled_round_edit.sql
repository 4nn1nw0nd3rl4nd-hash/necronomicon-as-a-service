-- Phase 3.3c2-1: one atomic GM edit, with pause/resume messages only.
...
begin
  ...
  -- Archive/unarchive remain allowed by the existing form and constraints.
  -- Archival messages are deliberately deferred to Phase 3.3c2-2.
...
revoke all on function public.update_round(uuid, text, text, text, text, text)
from public, anon;
grant execute on function public.update_round(uuid, text, text, text, text, text)
to authenticated;

-- A column revoke alone does not override a table-level UPDATE grant.
-- Remove both possible client status grants, including PUBLIC inheritance.
-- SECURITY DEFINER writers retain their owner's privileges; RLS is unchanged.
revoke update on table public.rounds from public, anon, authenticated;
revoke update (status) on table public.rounds from public, anon, authenticated;
grant update (name, system, description, appointment) on table public.rounds
to authenticated;
