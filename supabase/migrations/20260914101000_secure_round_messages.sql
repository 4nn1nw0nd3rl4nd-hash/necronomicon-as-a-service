-- Deliberately independent of can_view_round()/is_admin(): no admin content bypass.
create function public.can_read_round_messages(p_round_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    join public.rounds as round on round.id = membership.round_id
    where membership.round_id = p_round_id
      and membership.user_id = (select auth.uid())
      and (round.locked_at is null or membership.role = 'game_master')
  );
$$;

revoke all on function public.can_read_round_messages(uuid) from public, anon;
grant execute on function public.can_read_round_messages(uuid) to authenticated;
grant select on table public.round_messages to authenticated;

create policy "Current members can read round chat"
on public.round_messages for select to authenticated
using (public.can_read_round_messages(round_id));

create function public.send_round_message(
  p_round_id uuid,
  p_body text,
...
revoke all on function public.send_round_message(uuid, text, uuid, uuid) from public, anon;
grant execute on function public.send_round_message(uuid, text, uuid, uuid) to authenticated;
-- No INSERT/UPDATE/DELETE grants or policies for clients, and no mutating RPCs.
